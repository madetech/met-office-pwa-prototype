...
describe('Index', () => {
  ...
  it('should display forecast data place name', () => {
    render(<Index hasReadPermission={true} data={hourlyData} />);
  ...
  it("should display the forecast data's latitude and longitude", () => {
    ...
    const latitudeAndLongitude = screen.getByText(
      `Lat: ${hourlyData.features[0].geometry.coordinates[1]} / Long: ${hourlyData.features[0].geometry.coordinates[0]}`
    );

    expect(latitudeAndLongitude).toBeInTheDocument();
  });

  it('should display each hourly forecast time', () => {
    render(<Index hasReadPermission={true} data={hourlyData} />);

    hourlyData.features[0].properties.timeSeries.forEach((hourlyForecast) => {
      const hourlyData = screen.getByTestId(hourlyForecast.time);

      expect(hourlyData).toHaveTextContent(hourlyForecast.time);
    });
  });
  ...
  it('should display each hourly forecast temperature', () => {
    ...
    hourlyData.features[0].properties.timeSeries.forEach((hourlyForecast) => {
      ...
      expect(hourlyData).toHaveTextContent(
        hourlyForecast.screenTemperature.toString()
      );
    });
  });

  it('should display each hourly forecast precipitation probability', () => {
    render(<Index hasReadPermission={true} data={hourlyData} />);
    ...
    hourlyData.features[0].properties.timeSeries.forEach((hourlyForecast) => {
      ...
      expect(hourlyData).toHaveTextContent(
        hourlyForecast.probOfPrecipitation.toString()
      );
    });
  });
  ...
  it('should display each hourly forecast weather code', () => {
    render(<Index hasReadPermission={true} data={hourlyData} />);
    ...
    hourlyData.features[0].properties.timeSeries.forEach((hourlyForecast) => {
      ...
      expect(hourlyData).toHaveTextContent(
        hourlyForecast.significantWeatherCode.toString()
      );
    });
  });
});
